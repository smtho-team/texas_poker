...
export default {
  ...
  setup(props) {
    ...
    const getDay = computed(() => (time) => {
      var minutes = 24 * 60;
      var day = time / minutes;
      return day < 1 ? time : day;
    });
    const getDayName = computed(() => (time) => {
      var minutes = 24 * 60;
      var day = time / minutes;
      return day < 1 ? "minutes" : day == 1 ? "day" : "days";
    });
    const toDecimal = computed(() => (num) => {
      if (num) {
    ...
    const longestHoldingPeriod = ref({});
    const shortestHoldingPeriod = ref({});
    const holdBlueChip = ref({});
    const priceToZero = ref({});
    const userLabel = ref({});
    const rank = ref();
    const generate = () => {
